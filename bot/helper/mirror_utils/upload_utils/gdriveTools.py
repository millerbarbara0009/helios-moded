    ...
    def drive_list(self, fileName, stopDup=False, noMulti=False, isRecursive=True, itemType=""):
        ...
        for index, parent_id in enumerate(DRIVES_IDS):
            ...
            if not response["files"] and noMulti:
                ...
                continue
            if not Title:
                msg += f'<h4>Search Result For <i> {fileName} </i> </h4><br><br><b><a href="https://t.me/samc_z"> SA Mirror </a></b> || <b><a href="https://t.me/sa_sajjad_2"> Owner </a></b> <br>'
                Title = True
    ...
    def count(self, link):
        ...
        try:
            ...
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                msg += f'<b>➜/clone: </b><code>{name}</code>'
                if mime_type is None:
                    mime_type = 'File'
                self.__total_files += 1
